...
def get_name(result_path):
    db = fn.Database()
    query = "select name from music where path == ?;"
    db.cur.execute(query, (result_path,))
...
def f(lst, result_path):
    val = get_name(result_path)
    return [i for i, v in lst.items() if val in v]
...
for input_filename, db_files in tqdm(input_dict.items()):
    ...
    print("\n")
    '''
    print(input_filename + "is ranked in ")
    print(str(f(sim_rank_in_vocal, input_filename))+" in vocal, "+"in chords, "+"in all")